...
class DataInspection:
    ...
    def ask_for_scatterplot(self) -> None:
        ...
        for idx, col in enumerate(numeric_cols, start=1):
            print(f"{idx}. {col}")
        col1_idx = (int(input("Select the first column for scatterplot (enter the number): ")) - 1) % len(numeric_cols)
        col2_idx = (int(input("Select the second column for scatterplot (enter the number): ")) - 1) % len(numeric_cols)
        self.plot_scatter(numeric_cols[col1_idx], numeric_cols[col2_idx])

    def ask_for_boxplot(self) -> None:
        numeric_cols = self.numeric_columns()
        ordinal_cols = [col for col in self.df.columns if self.df[col].nunique() < 20]
        print("Numerical Columns:")
        for idx, col in enumerate(numeric_cols, start=1):
            print(f"{idx}. {col}")
        ...
        for idx, col in enumerate(ordinal_cols, start=1):
            print(f"{idx}. {col}")
        num_col_idx = (int(input("Select a numerical column for boxplot (enter the number): ")) - 1) % len(numeric_cols)
        ord_col_idx = (int(input("Select an ordinal column for boxplot (enter the number): ")) - 1) % len(ordinal_cols)
    ...
    def ask_for_correlation(self, numeric_cols) -> int:
        print("Numerical Columns:")
        for idx, col in enumerate(numeric_cols, start=1):
            print(f"{idx}. {col}")
        len_ = len(self.df)
        col1_idx = (int(input("Select the first column for correlation (enter the number): ")) - 1) % len_
        col2_idx = (int(input("Select the second column for correlation (enter the number): ")) - 1) % len_
        correlation_value = self.df[numeric_cols[col1_idx]].corr(self.df[numeric_cols[col2_idx]])
        return correlation_value

    def ask_for_std(self, numeric_cols) -> int:
        print("Numerical Columns:")
        for idx, col in enumerate(numeric_cols, start=1):
            print(f"{idx}. {col}")
        col_idx = (int(input("Select a column for standard deviation (enter the number): ")) - 1) % len(numeric_cols)
        std_value = self.df[numeric_cols[col_idx]].std()
        return std_value

    def ask_for_kurtosis(self, numeric_cols) -> int:
        print("Numerical Columns:")
        for idx, col in enumerate(numeric_cols, start=1):
            print(f"{idx}. {col}")
        col_idx = (int(input("Select a column for kurtosis (enter the number): ")) - 1) % len(numeric_cols)
        kurt_value = self.df[numeric_cols[col_idx]].kurt()
        return kurt_value

    def ask_for_skewness(self, numeric_cols) -> int:
        print("Numerical Columns:")
        for idx, col in enumerate(numeric_cols, start=1):
            print(f"{idx}. {col}")
